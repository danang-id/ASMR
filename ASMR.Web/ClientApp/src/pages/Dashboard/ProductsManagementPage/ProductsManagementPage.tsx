...
function ProductsManagementPage(): JSX.Element {
	...
	function onShowProductInformationPageButtonClicked(product: Product) {
		window.open(`${PublicRoutes.ProductInformationPage}/${product.id}`, "_blank")
	}
	...
	async function onShowProductCreateModalButtonClicked() {
		await singleSwitchToggle(setProductCreateModalShown, [
			setProductUpdateModalShown,
			setProductRemoveModalShown
		])
	}
	...
	async function onShowProductUpdateModalButtonClicked(Product: Product) {
		...
		await singleSwitchToggle(setProductUpdateModalShown, [
			setProductCreateModalShown,
			setProductRemoveModalShown
		])
	}
	...
	async function onShowProductRemoveModalButtonClicked(Product: Product) {
		setSelectedProduct(Product)
		await singleSwitchToggle(setProductRemoveModalShown, [
			setProductCreateModalShown,
			setProductUpdateModalShown
		])
	}
			...
			<Table.Row key={index}>
				<Table.DataCell>
					<div className="information">
						<div className="weight-per-packaging">
							{toLocaleUnit(product.weightPerPackaging, "gram")}
						</div>
						<BreakpointRenderer max="xs">
							<div className="price">
								{toLocalCurrency(product.price)}
							</div>
							<div className="product-inventory">
								In Inventory: {product.currentInventoryQuantity}
							</div>
		...
		<DashboardLayout className="page">
			...
			<div className="click-to-actions">
				...
				<div className="bean-selector">
					<ListSelector emptyOption="No beans available"
									onChange={setBean}
									value={bean}
									valueRenderer={beanValueRenderer}>
						{beans.length > 0 && beans.map((value, index) => (
			...
			<div className="content">
				<div className="table-container">
					<Table>
						...
						<Table.Body>
							{products.length > 0 ? products.map(renderProductTableRow) : (
								<Table.Row>
									<Table.DataCell colSpan={breakpoint.includes("sm") ? 4 : 1}>
										<span className="table-data-information">
											{
												progress.loading  ? "Retrieving data from server..."
													: beans.length <= 0 ? "There are no beans at the moment. You may add bean from Bean Management Panel."
														: bean ? `There are no products for ${bean.name} bean at the moment.`
			...
			<ProductCreateModal bean={bean}
								onClose={onCloseModals}
								onCreateProduct={createProduct}
								progress={progress}
								show={productCreateModalShown} />
			<ProductUpdateModal bean={bean}
								product={selectedProduct}
								onClose={onCloseModals}
								onUpdateProduct={updateProduct}
								progress={progress}
								show={productUpdateModalShown} />
			<ProductRemoveModal bean={bean}
								product={selectedProduct}
								onClose={onCloseModals}
