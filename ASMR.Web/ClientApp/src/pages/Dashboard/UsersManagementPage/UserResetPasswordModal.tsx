...
function UserUpdatePasswordModal({ onClose, onResetUserPassword, progress, show, user }: UserResetPasswordModalProps): JSX.Element {
	return (
		<Modal onClose={onClose} show={show} title={`Reset Password for ${user?.firstName ?? ""} ${user?.lastName ?? ""}`}>
			<Modal.Body>
				{
					user?.isEmailConfirmed ? (
						...
						<div className="reset-user-password-modal-body">
							<p>
								The email address <span className="email-address">{user?.emailAddress}</span>{" "}
								has not been confirmed. Password reset procedure cannot continue.
							</p>
						</div>
					)
				}
			...
			<Modal.Footer>
				{
					user?.isEmailConfirmed ? (
						<div className="modal-actions">
							...
							<Button disabled={progress.loading} size="sm" onClick={() => onResetUserPassword()}>Send Reset Instruction</Button>
						</div>
					) : (
						<div className="modal-actions">
							<Button disabled={progress.loading} size="sm" onClick={onClose}>Okay</Button>
						</div>
					)
				}
			</Modal.Footer>
