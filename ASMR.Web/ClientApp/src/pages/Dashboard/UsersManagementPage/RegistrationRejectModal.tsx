...
function RegistrationRejectModal({ onClose, onRejectRegistration, progress, show, user }: RegistrationRejectModalProps): JSX.Element {
	return (
		<Modal onClose={onClose} show={show} title={`Reject Registration of ${user?.firstName ?? ""} ${user?.lastName ?? ""}`}>
			<Modal.Body>
				<div className="reject-registration-modal-body">
					<p>
						The following account registration will be rejected.<br/>
			...
			<Modal.Footer>
				<div className="modal-actions">
					...
					<Button disabled={progress.loading} style="danger" size="sm" onClick={onRejectRegistration}>Reject</Button>
				</div>
			</Modal.Footer>
		</Modal>
	)
