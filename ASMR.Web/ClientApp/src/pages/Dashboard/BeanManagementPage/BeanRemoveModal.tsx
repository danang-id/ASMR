		...
		<Modal onClose={onClose} show={show} title={`Remove ${bean?.name ?? ""}`}>
			<Modal.Body>
				<p className="remove-bean-modal-body">
					Are you sure you would like to remove the bean <span className="name">{bean?.name ?? ""}</span> from
					the bean list?
				</p>
			</Modal.Body>
			...
			<Modal.Footer>
				<div className="modal-actions">
					...
					<Button disabled={progress.loading} style="danger" size="sm" onClick={onRemoveBean}>
						Remove
					</Button>
				</div>
			</Modal.Footer>
